...
- **공개 페이지 테마는 포스터를 따라간다** (애플 뮤직 방식). `accentFromImage()` →
  ...
  - **채도는 어두운 톤에서 살리고(≤.75) 밝은 톤에서만 옅게(×.5)**. 어두운 쪽까지 깎으면 회색빛으로 칙칙해지고,
    밝은 쪽에 하한을 두면 흰 포스터에 파란 배경이 깔린다 — 둘 다 겪었다. 하한은 버튼에만 둔다(≥.35)
  ...
  - 버튼은 어두운 배경에서 명도 .80 부터 시작해 **대비 4.5:1** 이 될 때까지 명도만 민다 (`tkFitContrast`).
    중간 톤이면 배경에 가라앉는다 — 애플 뮤직의 밝은 알약 버튼처럼 파스텔에 가깝게 둔다. 글자는 흰/검 중 대비가 큰 쪽 (`tkInkFor`)
  - 무채색 포스터는 색을 지어내지 않고 밝기만 따라간다
- **관리자 미리보기와 공개 페이지는 CSS 가 같아야 한다** — 여백·`background-attachment` 가 달라
  "미리보기랑 실제가 다르다"가 됐었다. `.tkp-phone .tkp-page` 는 공개 페이지 값을 그대로 쓰고
  `background-attachment` 만 `scroll` 로 (미리보기는 뷰포트가 아니라 박스 안에서 스크롤한다)
